...
export const api_notifications = {
  notification: {
    ...
    postUnfollow: async (followedId, followerId) => {
      ...
      try {
        ...
        console.log(
          'Notificação de unfollow adicionada com sucesso:',
          docRef.id
        )
      } catch (error) {
    ...
    hasAlreadyBeenSeen: async (userId, notificationId) => {
      ...
      try {
        ...
        console.error(
          'Erro ao verificar se as notificações foram lidas:',
          error
        )
        return false
      }
      ...
      try {
        ...
        const updatePromises = querySnapshot.docs.map(async (doc) => {
          ...
          if (notificationData.data.isRead === false) {
            ...
            notificationData.data.alreadyOpenedHours =
              new Date().toLocaleTimeString()
            notificationData.data.alreadyOpenedDate =
        ...
        console.log(
          'Notificações do usuário marcadas como lidas com sucesso:',
          userId
        )
      } catch (error) {
        console.error(
          'Erro ao marcar notificações do usuário como lidas:',
          error
        )
      }
    },
  ...
  notificationCreateRecipe: {
    newRecipe: async (docRefId) => {
      if (!docRefId) {
        console.error(
          'Parâmetros inválidos para criar notificação de nova receita.'
        )
        return
      }
      try {
        ...
        const notificacoesPromises = seguidores.map(async (shotId) => {
          const notificationData = {
            ...
            data: {
              title: 'Nova Receita',
              action: `${seguidoresData.name} criou uma nova receita: ${
                receitaDataRef.data().recipeTitle
              }`,
              isRead: false,
              alreadyOpenedHours: '',
              alreadyOpenedDate: '',
